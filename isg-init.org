  ...
  | C-x C-q | ivy-wgrep-change-to-wgrep-mode           | make search results editable! |
...
(defun isg/machine-settings ()
  ...
  (cond
   ((string-match "^BERTRAND" system-name)  ; 2016 Windows PC
    '((default-font "-outline-Inconsolata-normal-normal-normal-mono-13-*-*-*-m-0-iso10646-1")
      ...
      (frame-l ((top . 0) (left . 843) (width . 120) (height . 83)))
      (frame-r ((top . 0) (left . 1700) (width . 120) (height . 83)))))
...
*** installation notes
    <2017-10-01 Sun> installed haskell on chromebook
    ...
    fixed with the following env change:
    $ export CURL_CA_BUNDLE=/etc/ssl/certs/ca-certificates.crt
    $ curl -sSL https://get.haskellstack.org/ | sh

    had a warning that ~/.local/bin wasn't in PATH so added that to ~/.profile

    _the binary 'stack' was installed as /usr/local/bin/stack_

    $ stack new my-project (takes fucking ages)
    $ cd my-project
    $ stack setup (installs GHC - takes fucking ages) - RAN OUT OF DISK SPACE - tried again
    $ stack build
    $ stack exec my-project-exe

    Note: here's my ~/.stack/config.yaml file:

 templates:
   params:
     author-name: Inderjit Gill
     author-email: email@indy.io
     copyright: 'Copyright: (c) 2017 Inderjit Gill'
     github-username: indy

   <2017-10-03 Tue> installed haskell on Windows
   The installer added a STACK_ROOT environment variable. 
   It's the equivalent of ~/.stack on Unix and is set to C:\sr
